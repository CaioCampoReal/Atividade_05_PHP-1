...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['criar'])) {
        ...
        echo $_POST['data_velorio'] . " ";
        echo $_POST['finado_id'] . " ";
        echo $_POST['urna_id']. " ";
...
function exibirModalVelorio($modo = 'criar', $dados = null) {
    ...
    $titulo = ($modo == 'criar') ? 'Criar Velório' : 'Editar Velório';
                ...
                <div class="modal-body">
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="finado_id">';
                                global $listaFinados;
                                foreach ($listaFinados as $finado) {
                                    echo '<option value="'.$finado->getFinadoId().'">'.$finado->getFinadoNome().'</option>';
                                }
    echo                    '</select>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="urna_id">';
                                ...
                                foreach ($listaUrnas as $urna) {
                                    echo '<option value="'.$urna->getUrnaId().'">'.$urna->getUrnaMaterial().'</option>';
                                }
    echo                    '</select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Data Velório</label>
                            <input type="date" class="form-control" name="data_velorio" required>
                        </div>
            ...
            <tbody>
                <?php foreach ($listaVelorios as $velorio): ?>
                    <tr>
                        ...
                        <td><?= $velorio->getVelorioData() ?></td>
                        <td><?=  $velorio->getUrna()->getUrnaTipo() ?></td>
                        <td>R$ <?= number_format($velorio->getUrna()->getUrnaPreco(), 2, ',', '.') ?></td>
                        <td>
